...
# --------------------------------------------------------------------------- #
# rollout / end-to-end (needs p08 simulator merged)
# --------------------------------------------------------------------------- #
@pytest.mark.xfail(reason="needs p08", strict=False)
def test_evaluate_end_to_end():
    decision = {
        "start": {"city": "New York", "cash": 100000},
        "actions": [("buy", "Cocaine", 100), ("sell", "Cocaine", 100)],
        "resolve_after_days": 2,
    }
    s = evaluate(decision, n=200, base_seed=0)
    assert s["n"] == 200
    assert set(s) == {"ev", "p10", "p50", "p90", "p_loss", "n"}
    # a sane distribution: ordered percentiles, ev inside the p10..p90 band
    assert s["p10"] < s["p50"] < s["p90"]
